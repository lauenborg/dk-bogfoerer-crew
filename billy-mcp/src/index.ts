      ...
      for (const assoc of assocs) {
        ...
        if (delErr) {
          ...
          if (ref.startsWith("daybookTransaction:")) {
            ...
            // VOID — aldrig DELETE (sletter bilag!). Void bevarer bilag.
            const { error: voidErr } = await safeCall(() => voidBill(billId));
            parts.push(voidErr ? `✗ Void regning: ${voidErr}` : `✓ Regning voided (bilag bevaret): ${billId}`);
